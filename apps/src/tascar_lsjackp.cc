...
int main(int argc, char** argv)
{
  ...
  struct option long_options[] = {
      {"help", 0, 0, 'h'},  {"jackname", 1, 0, 'j'}, {"output", 0, 0, 'o'},
      {"input", 0, 0, 'i'}, {"physical", 0, 0, 'p'}, {"soft", 0, 0, 's'},
      {0, 0, 0, 0}};
  ...
  jack_client_t* jc(
      jack_client_open(jackname.c_str(), JackNoStartServer, NULL));
  if(jc) {
    const char** ports(jack_get_ports(jc, NULL, NULL, jackflags));
    if(ports) {
      while(*ports) {
