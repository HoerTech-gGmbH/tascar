...
public:
  ...
  receiverexample_t(tsccfg::node_t xmlsrc);
...
receiverexample_t::data_t::data_t() : weight(0) {}

receiverexample_t::receiverexample_t(tsccfg::node_t xmlsrc)
    : TASCAR::receivermod_base_t(xmlsrc),
      start_angle(10 * DEG2RAD) // default value is 10 degrees
{
  // register XML variable:
  GET_ATTRIBUTE_DEG(start_angle, "start angle");
}
...
void receiverexample_t::add_pointsource(const TASCAR::pos_t& prel, double width,
                                        const TASCAR::wave_t& chunk,
                                        std::vector<TASCAR::wave_t>& output,
...
void receiverexample_t::add_diffuse_sound_field(
...
receiverexample_t::create_state_data(double srate, uint32_t fragsize) const
{
  return new data_t();
}
