...
public:
  ...
  sourceexample_t(tsccfg::node_t xmlsrc);
...
sourceexample_t::data_t::data_t() : weight(0) {}

sourceexample_t::sourceexample_t(tsccfg::node_t xmlsrc)
    : TASCAR::sourcemod_base_t(xmlsrc),
      start_angle(10 * DEG2RAD) // default value is 10 degrees
{
  // register XML variable:
  GET_ATTRIBUTE_DEG(start_angle, "starting angle");
}
...
bool sourceexample_t::read_source(TASCAR::pos_t& prel,
                                  const std::vector<TASCAR::wave_t>& input,
...
void sourceexample_t::configure()
{
  ...
  if(n_channels != 1)
    throw TASCAR::ErrMsg(
        "his source model requires one input channel, received " +
        TASCAR::to_string(n_channels) + ".");
}

TASCAR::sourcemod_base_t::data_t*
sourceexample_t::create_state_data(double srate, uint32_t fragsize) const
{
  return new data_t();
}
