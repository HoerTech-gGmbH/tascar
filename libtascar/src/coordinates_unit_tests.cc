...
TEST(pos_t,set_sphere)
{
  ...
  pos.set_sphere(1.0,TASCAR_PI,0.5*TASCAR_PI);
  ...
  pos.set_sphere(1.0,0.5*TASCAR_PI,-0.25*TASCAR_PI);
...
TEST(quaternion_t,rotate)
{
  ...
  q.set_rotation( 0.5*TASCAR_PI, TASCAR::pos_t( 0, 0, 1 ) );
  ...
  qp.set_rotation( 0.5*TASCAR_PI, TASCAR::pos_t( 0, 0, 1 ) );
  qp *= q;
  ASSERT_NEAR( cosf( 0.5*TASCAR_PI ), qp.w, 1e-7 );
  ASSERT_NEAR( 0.0f, qp.x, 1e-8 );
  ASSERT_NEAR( 0.0f, qp.y, 1e-8 );
  ASSERT_NEAR( sinf( 0.5*TASCAR_PI ), qp.z, 1e-7 );
  p = TASCAR::pos_t( 1, 0, 0 );
  qp.rotate( p );
