...
\indattr{buttonheight} & Button height in pixels                               \\
\end{tscattributes}
...
Within a preset, a number of \elem{osc} elements can be defined. These attributes are supported:
\begin{tscattributes}
\indattr{path} & OSC varaibale path, either ``/pos'' or ``/zyxeuler'' are appended \\
\indattr{pos}  & Position x y z Cartesian coordinates in meter (optional).         \\
\indattr{rot}  & Orientation z y x Euler angles in degree (optional).              \\
\end{tscattributes}
