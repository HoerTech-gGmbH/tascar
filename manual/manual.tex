...
\newcommand{\refelem}[2][]{\ifthenelse{\equal{#1}{}}{\hyperref[sec:#2]{\elembox{<#2/>}}}{\hyperref[sec:#1]{\elembox{<#2/>}}}\index{#2}}
...
(\url{http://jackaudio.org}). It is required to start jack
before loading a session into \tascar{}.
...
representation of this scene definition in \tascar{}. The main window\index{main window}
contains a toolbar for file interactions, transport and time control,
and controls for muting and soloing the different components of the
scene (left panel). The scene map window contains a visual
representation of the scene. Editing the scene via the graphical user
interface is currently not possible.
...
\indattr{mirrororder} & Order of image source model (default: 1)                  \\
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=0.8\textwidth]{connectsounds.png}}
...
(deprecated for source objects, use only for diffuse objects -- see
\refelem[ap_sndfile]{sndfile} audio plugin instead)

A sound content can be assigned to a diffuse sound field using a
child element \elem{sndfile}.
%
...
\begin{figure}[htb]
  \centering
  \fbox{\includegraphics[width=0.8\textwidth]{player.png}}
...
\begin{tscelements}
\refelem{position}, \refelem{boundingbox},\refelem{orientation}, \refelem{creator}, \refelem{sndfile}
\end{tscelements}

Apart of sound sources which consist of one or more sound vertices, we
also have a possibility to create diffuse sound sources which "fill"
the space and are equally loud within a given volume (for example
stationary diffuse noise from ventilation). 
%
...
(first order ambisonics format with wxyz channel order), so we also
have to remember to specify the number of channels by using
\attr{channels="4"} attribute. 
%
With attribute \attr{size="x y z"} we can specify the dimensions of
the box in which the diffuse sound is audible. 
%
...
four renderer input ports (which are always generated with the diffuse sound field:  \verb!render.<scenename>:<objectname>.n!) are connected to the audio player output ports \verb!@.0!, \verb!@.1!,
\verb!@.2! and \verb!@.3!, where ''@'' is \verb!player.<scenename>:<objectname>! . Example : \tscexample[linerange={5-7},firstnumber=5]{example_diffuse}
...
\indattr{size}         & Size of box in which no distance-rule is applied (x,y,z in m); default: 0 0 0        \\
...
decoder matrix. To achieve a $\max rE$-decoder, set the attribute
\indattr{xyzgain} to 0.707 for regular horizontal speaker layouts and
to 0.577 for regular 3D speaker layouts.  For Ambisonics based
receiver types, $D$ is a diagonal matrix.
%
...
The speaker layout of speaker based receiver types can be either
defined in a separate layout file, or in a list of \elem{speaker}
elements within the receiver definition.
...
\indattr{connect} & Connection to jack port                                   \\
\indattr{compA},
\indattr{compB} & IIR filter coefficients for speaker calibration. \\
\end{tscattributes}

To activate IIR speaker correction filter, provide both \attr{compA} and
\attr{compB} attributes.
...
{\tt vbap}            & user def. & see \cite{Pulkki1997} for details.                                                                                                             \\
...
%
Speaker-based receiver types have these additional configuration variables:

\begin{tscattributes}
\indattr{layout}         & File name of speaker layout definition                                       \\
...
\tscexample[linerange={4-11},firstnumber=4]{example_nearest}
...
%\begin{figure}[ht]
%\centerline{\includegraphics[width=\columnwidth]{filestructure.png}}
...
Each sound vertex \elem{sound} and each receiver \elem{receiver} can
contain a list of audio plugins for processing and analysis, such as
tone generators or speech analysis for lip synchronization
modeling. These audio plugins are specified within the \elem{plugins} section within a \elem{sound} or \elem{receiver} element, e.g.:
...
\subsection{sndfile}\label{sec:ap_sndfile}
...
\attr{maxspeechlevel} & Level normalization in dB                                                    \\
\attr{dynamicrange}   & Mapped dynamic range in dB                                                   \\
...
source and receiver. In the level meters, root-mean-square (RMS)
...
%%  LocalWords:  TASCARpro Ambisonics binaural reflectivity facegroup
