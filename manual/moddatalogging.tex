\subsection{datalogging}\label{sec:datalogging}\index{datalogging}
...
\begin{tscattributes}
  \attr{port}       & OSC port (default: empty, i.e., use session server)\\
  \attr{multicast}  & OSC multicasting address (default: empty)\\
  \attr{fileformat} & File format, can be either ``mat'', ``matcell'' or ``txt'' (default: ``mat'')\\
  \attr{outputdir}  & Output directory (default: empty)\\
  \attr{lsltimeout} & Number of seconds to scan for LSL streams (default: 10)\\
  \attr{displaydc}  & Display DC components (default: true)\\
\end{tscattributes}
...
structure, with a \attr{name} field, a \attr{data} field, and for LSL
