...
eog_t::eog_t(const sensorplugin_cfg_t& cfg)
    : sensorplugin_drawing_t(cfg), device("/dev/rfcomm1"), baudrate(38400),
      charsize(8), offset(512.0), scale(3.3 / 1024.0),
      // range(1.0),
      unit("mV"), dev(NULL), timeoutcnt(0), lval(0), baud(B9600), warncnt(0)
{
  range.push_back(0);
  ...
  case 230400:
    ...
    break;
  default:
    throw ErrMsg("Invalid baud rate.");
  }
  lsl::stream_info streaminfo(get_name(), modname, 1, lsl::IRREGULAR_RATE,
                              lsl::cf_double64);
...
void eog_t::draw(const Cairo::RefPtr<Cairo::Context>& cr, double width,
                 double height)
{
