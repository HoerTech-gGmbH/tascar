...
midicc_t::midicc_t(const sensorplugin_cfg_t& cfg)
    : sensorplugin_drawing_t(cfg), TASCAR::midi_ctl_t("glabsensor" +
                                                      cfg.modname),
      run_service(true)
{
  if(name.empty())
  ...
  lsl::stream_info streaminfo(get_name(), modname, controllers_.size(), 100.0,
                              lsl::cf_double64);
  ...
  for(uint32_t k = 0; k < controllers_.size(); ++k) {
    uint8_t v(std::max(
        0.0,
        std::min(127.0, (data[k] - range[0]) / (range[1] - range[0]) * 127.0)));
...
void midicc_t::draw(const Cairo::RefPtr<Cairo::Context>& cr, double width,
                    double height)
{
  cr->save();
  ...
  for(uint32_t k = 0; k < controllers_.size(); ++k) {
    ...
    cr->stroke();
    char ctmp[256];
    snprintf(ctmp, sizeof(ctmp), "%d/%d", controllers_[k] >> 8,
             controllers_[k] & 0xff);
  ...
  for(uint32_t k = 0; k < ldata.size(); ++k) {
    if(ldata[k] != HUGE_VAL) {
      ...
      double y1(y0 -
                (height - 40) * (ldata[k] - range[0]) / (range[1] - range[0]));
      cr->move_to(x, y0);
      cr->line_to(x, y1);
      cr->stroke();
    }
