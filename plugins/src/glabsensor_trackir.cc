...
void zyx_euler2mat( const TASCAR::zyx_euler_t& eul, cv::Mat& mat )
{
  mat =  
    // rot_z:
    (cv::Mat_<double>(3,3) << 
     cos(eul.z), -sin(eul.z), 0, 
     sin(eul.z), cos(eul.z), 0, 
     0, 0, 1) *
    // rot_y:
    (cv::Mat_<double>(3,3) << 
     cos(eul.y), 0, sin(eul.y), 
     0, 1, 0, 
     -sin(eul.y), 0, cos(eul.y)) *
    // rot_x:
    (cv::Mat_<double>(3,3) << 
     1, 0, 0, 
     0, cos(eul.x), -sin(eul.x), 
     0, sin(eul.x), cos(eul.x));
}
...
/**
  ...
  \retval r resulting ratio of intersection point; values between 0 and 1 are within the line
  \return orthogonal distance from the line
 */
double distance_from_line( const std::vector<std::complex<double> >& vP, uint32_t k1, uint32_t k2, uint32_t k3, double& r )
{
 ...
 */
uint32_t num_lines( const std::vector<std::complex<double> >& vP, uint32_t k, double threshold, std::vector<uint32_t>& idx_end, std::vector<uint32_t>& idx_mid, std::vector<double>& vr )
{
  idx_end.resize(0);
  idx_mid.resize(0);
  vr.resize(0);
...
data_t::data_t()
  : vP(NUM_MARKER,0.0),
    ...
    cIdx_mid(NUM_MARKER,std::vector<uint32_t>(NUM_MARKER,0)),
    ageraw(-1),
    agesolved(-1)
{
}
...
public:
  class error_t {
  public:
    error_t(double e_=0,uint32_t k0_=0,uint32_t k1_=0):e(e_),k0(k0_),k1(k1_){};
    double e;
    uint32_t k0;
    uint32_t k1;
  };
...
bool err_comp( const trackir_tracker_t::error_t& a, const trackir_tracker_t::error_t& b)
{
  return a.e < b.e;
}
...
trackir_tracker_t::trackir_tracker_t( const sensorplugin_cfg_t& cfg )
  : sensorplugin_drawing_t(cfg),
    vMap(NUM_MARKER,error_t(0,0)),
    transform(6,0.0),
    transform1(6,0.0),
    info_marker(name+"marker","motrack",3*NUM_MARKER,120,lsl::cf_double64,name+cfg.hostname+"trackir5raw_t"+TASCARVER),
    outlet_marker(info_marker),
    info_transform(name,"motrack",6,120,lsl::cf_double64,cfg.hostname+"trackir5tf_t"+TASCARVER),
    outlet_transform(info_transform),
    info_transform1(name+"presolve","motrack",6,120,lsl::cf_double64,cfg.hostname+"trackir5tf1_t"+TASCARVER),
    outlet_transform1(info_transform1),
    vPN(NUM_MARKER,0.0),
  vN(NUM_MARKER,0),
  cR(NUM_MARKER,std::vector<double>(NUM_MARKER,0.0)),
  pC(HUGE_VAL),
  counter(0),
  idxLines(NUM_MARKER,0),
  idx2Lines(NUM_MARKER,0),
  idxNLines(NUM_MARKER,0),
  vStd(NUM_MARKER,0.0),
  scale(1.0),crot(1.0),
  rot(0.0),
  linethreshold(1.0),
    ...
    if( counter != pose.counter ){
      ...
      if( blobs_read >3 ){
        ...
        for(int32_t k=0;k<blobs_read;++k){
          vP[k] = (1.0-2.0*(double)flipx)*blobs[3*k]+1.0i*(1.0-2.0*(double)flipy)*blobs[3*k+1];
        }
        marker_match();
...
std::complex<double> get_median_point(const std::vector<std::complex<double> >& vP )
{
    ...
    if( idx2Lines.size() == 0 ){
	  ...
	  vStd.resize(idxNLines.size());
	    ...
	    double mean(std::accumulate(acc.begin(),acc.end(),0.0)/acc.size());
            vStd[l] = std::sqrt(std::inner_product(acc.begin(),acc.end(),acc.begin(),0.0)/acc.size()-mean*mean);
	  }
          kC = idxNLines[std::min_element(vStd.begin(),vStd.end())-vStd.begin()];
	}else{
	  ...
	  std::vector<double> vDist(idxLines.size());
	    ...
	    std::vector<double>::iterator dmini(std::min_element(vDist.begin(),vDist.end()));
	    kC = idxLines[dmini-vDist.begin()];
          }
	}
      }
...
Resulting numbers are stored in vN, resulting end point lists are stored in cIdx.
*/
...
void trackir_solver_t::load_camcalib()
{
  ...
  if( !calibfile.good() )
    throw TASCAR::ErrMsg("Unable to open calib file \""+TASCAR::env_expand(camcalibfile)+"\".");
  ...
  if( calibvals.size() != 6 ){
    ...
    throw TASCAR::ErrMsg("Invalid number of calibration values in file "+camcalibfile);
  }
  std::lock_guard<std::mutex> lock(mtx);
  tf_cam.position = TASCAR::pos_t(calibvals[0],calibvals[1],calibvals[2]);
  tf_cam.orientation = TASCAR::zyx_euler_t(DEG2RAD*calibvals[3],DEG2RAD*calibvals[4],DEG2RAD*calibvals[5]);
  zyx_euler_inv2mat( tf_cam.orientation, cam_comp_mat );
}
...
void trackir_solver_t::load_crown()
{
  ...
  if( !fcrownfile.good() )
    throw TASCAR::ErrMsg("Unable to open crown file \""+TASCAR::env_expand(crownfile)+"\".");
  ...
  while(!fcrownfile.eof() ){
    ...
    if( !calibline.empty() ){
      std::vector<double> linevals(TASCAR::str2vecdouble(calibline));
      crown_coordinates.insert(crown_coordinates.end(),linevals.begin(),linevals.end());
    }
  }
  ...
  for( uint32_t k=0;k<6;++k){
    crown3D[k] = cv::Point3f(crown_coordinates[3*k],crown_coordinates[3*k+1],crown_coordinates[3*k+2]);
    crown2D[k] = cv::Point2f(crown_coordinates[3*k],crown_coordinates[3*k+1]);
...
trackir_drawer_t::trackir_drawer_t( const sensorplugin_cfg_t& cfg )
  : trackir_solver_t( cfg ),
    camview(true),
    b_calib("re-calibrate"),
    b_calibsave("save calibration"),
    b_calibload("load calibration"),
    b_calibreset("reset calibration"),
    ...
    b_drawraw("camera view")
{
  ...
  b_calib.signal_clicked().connect(sigc::mem_fun(*this,&trackir_drawer_t::on_click_calib));
  b_calibsave.signal_clicked().connect(sigc::mem_fun(*this,&trackir_drawer_t::on_click_calib_save));
  b_calibload.signal_clicked().connect(sigc::mem_fun(*this,&trackir_drawer_t::on_click_calib_load));
  b_calibreset.signal_clicked().connect(sigc::mem_fun(*this,&trackir_drawer_t::on_click_calib_reset));
  b_calibuse.signal_clicked().connect(sigc::mem_fun(*this,&trackir_drawer_t::on_click_calib_use));
  b_drawraw.signal_clicked().connect(sigc::mem_fun(*this,&trackir_drawer_t::on_click_drawraw));
...
void trackir_solver_t::solve()
{
  ...
  for(uint32_t k=0;k<vMap.size();++k){
    imagePoints[k] = cv::Point2f(std::real(vP[vMap[k].k1]),std::imag(vP[vMap[k].k1]));
  ...
  try{
    ...
    bool res(cv::solvePnP( objectPoints, imagePoints, cameraMatrix, distCoeffs, rvec, tvec ));
    //bool res(false);
    if( res ){
...
{
  char c(axes[2*ax]);
  return 2.0*ax-2.0+(c=='x')*pos.x+(c=='y')*pos.y+(c=='z')*pos.z;
}
...
void trackir_drawer_t::draw(const Cairo::RefPtr<Cairo::Context>& cr, double width, double height)
{
  ...
  if( camview ){
    ...
    cr->rectangle(-320+margin,-240+margin,640-2*margin,480-2*margin);
    cr->stroke();
    cr->restore();
    cr->save();
    ...
    if( plot.ageraw < PLOT_AGELIMIT ){
      ...
      for( std::vector<std::complex<double> >::const_iterator ip=plot.vP.begin();ip!=plot.vP.end();++ip){
    ...
    if( plot.agesolved < PLOT_AGELIMIT ){
      ...
      cr->set_font_size( 20 );
      for(uint32_t k=0;k<std::min(plot.imagePoints.size(),plot.crownIndex.size());++k){
    ...
    if( plot.agesolved < PLOT_AGELIMIT ){
      ...
      campos *= -1.0;
      for(std::vector<pos_t>::iterator it=pcam.begin();it!=pcam.end();++it){
      ...
      for(int32_t kax=0;kax<3;++kax){
        ...
        cr->stroke();
        cr->set_source_rgba( (axes[2*kax] == 'x'), (axes[2*kax] == 'y'), (axes[2*kax] == 'z'), 0.5 );
        cr->move_to( -2+2*kax, 0 );
        cr->line_to( -1+2*kax, 0 );
        cr->stroke();
        cr->set_source_rgba( (axes[2*kax+1] == 'x'), (axes[2*kax+1] == 'y'), (axes[2*kax+1] == 'z'), 0.5 );
        cr->move_to( -2+2*kax, 0 );
