...
public:
  ...
  double alphamin;         // parameter which determines maximal depth of SHM at thetamin
  double startangle_front; // define range in which filter modelling pinna shadow operates
  double omega_front;      // cut-off frequency of pinna shadow filter
  double alphamin_front;   // parameter which determines maximal depth of pinna shadow filter
  double startangle_up;    // define range in which filter modelling torso shadow operates
  double omega_up;         // cut-off frequency of torso shadow filter
  double alphamin_up;      // parameter which determines maximal depth of torso shadow filter
  double startangle_notch; // define range in which parametric equalizer operates
  double freq_start;       // notch frequency at startangle_notch
  double freq_end;         // notch frequency at 90 degr elev
  double maxgain;          // gain applied at 90 degr elev (0 dB gain at startangle_notch and linear increase)
...
hrtf_param_t::hrtf_param_t( xmlpp::Element* xmlsrc )
  : TASCAR::xml_element_t(xmlsrc),
    ...
    omega_front(11200),
    alphamin_front(0.39),
    startangle_up(135*DEG2RAD),
//    omega_up(c/radius/2),
    alphamin_up(0.1),
    startangle_notch(102*DEG2RAD),
    freq_start(1300),
    freq_end(650),
    maxgain(-5.4),
    Q_notch(2.3)
...
public:
  ...
  public:
    ...
    void filterdesign( const double theta_left, const double theta_right, const double theta_front, const double elevation );
...
hrtf_t::data_t::data_t(double srate,uint32_t chunksize,hrtf_param_t& par_plugin)
  : fs(srate),
    dt(1.0/std::max(1.0,(double)chunksize)),
    par(par_plugin),
    dline_l(4*par.radius*srate/par.c+2+par.sincorder,srate,par.c,par.sincorder,64),
    dline_r(4*par.radius*srate/par.c+2+par.sincorder,srate,par.c,par.sincorder,64),
...
void hrtf_t::data_t::filterdesign( const double theta_l, const double theta_r, const double theta_f, const double elevation )
{
  // bqazim_l/r
  // parameter of SHM
  alpha_l = (1.0 + 0.5*par.alphamin + (1.0 - 0.5*par.alphamin)*cos(theta_l/par.thetamin*M_PI) ) * fs; 
  alpha_r = (1.0 + 0.5*par.alphamin + (1.0 - 0.5*par.alphamin)*cos(theta_r/par.thetamin*M_PI) ) * fs; 

  if( theta_f > par.startangle_front )  // pinna shadow effect
    {
      alpha_f = (1.0 -(1.0-par.alphamin_front) * (0.5-cos( (theta_f-par.startangle_front)/(M_PI-par.startangle_front) *M_PI) *0.5)) * fs;
      //alpha_f = (1.0 - ( 1.0-par.alphamin_front ) * ( 0.5-0.5*cos(theta_f) ) ) *fs; // if startangle_front == 0

      bqazim_l.set_coefficients( (par.omega-fs)*inv_a0 + (par.omega_front-fs)*inv_a0_f, (par.omega-fs)*inv_a0*(par.omega_front-fs)*inv_a0_f, 
        (par.omega+alpha_l)*inv_a0*(par.omega_front+alpha_f)*inv_a0_f, (par.omega+alpha_l)*inv_a0*(par.omega_front-alpha_f)*inv_a0_f + (par.omega-alpha_l)*inv_a0*(par.omega_front+alpha_f)*inv_a0_f, (par.omega-alpha_l)*inv_a0*(par.omega_front-alpha_f)*inv_a0_f );

      bqazim_r.set_coefficients( (par.omega-fs)*inv_a0 + (par.omega_front-fs)*inv_a0_f, (par.omega-fs)*inv_a0*(par.omega_front-fs)*inv_a0_f,
        (par.omega+alpha_r)*inv_a0*(par.omega_front+alpha_f)*inv_a0_f, (par.omega+alpha_r)*inv_a0*(par.omega_front-alpha_f)*inv_a0_f + (par.omega-alpha_r)*inv_a0*(par.omega_front+alpha_f)*inv_a0_f, (par.omega-alpha_r)*inv_a0*(par.omega_front-alpha_f)*inv_a0_f );
    }
    else
    {
      bqazim_l.set_coefficients( (par.omega-fs)*inv_a0, 0.0 , (par.omega+alpha_l)*inv_a0 , (par.omega-alpha_l)*inv_a0 , 0.0 );
      bqazim_r.set_coefficients( (par.omega-fs)*inv_a0, 0.0 , (par.omega+alpha_r)*inv_a0 , (par.omega-alpha_r)*inv_a0 , 0.0 );
    }

  // bqelev_l/r
  if ( elevation < par.startangle_notch )  // concha notch (parametric equalizer for cut)
    {
      double p_angle = (par.startangle_notch-elevation)/par.startangle_notch;
      double transform_const = 1.0 / tan( M_PI*( p_angle * (par.freq_end-par.freq_start) + par.freq_start ) / fs );
      ...
      bqelev_l.set_coefficients( 2.0*(1.0-tc_sq)*inv_a0_n, (tc_sq+1.0-inv_gain_Q)*inv_a0_n,
        (tc_sq+1.0+Q_n)*inv_a0_n, 2.0*(1.0-tc_sq)*inv_a0_n, (tc_sq+1.0-Q_n)*inv_a0_n );
      bqelev_r.set_coefficients( 2.0*(1.0-tc_sq)*inv_a0_n, (tc_sq+1.0-inv_gain_Q)*inv_a0_n,
        (tc_sq+1.0+Q_n)*inv_a0_n, 2.0*(1.0-tc_sq)*inv_a0_n, (tc_sq+1.0-Q_n)*inv_a0_n );
    }
    else if ( elevation > par.startangle_up )  // torso shadow effect
    {
      alpha_u = (1.0 - (1.0-par.alphamin_up) * (0.5 - 0.5*cos( (elevation-par.startangle_up)/(M_PI-par.startangle_up) *M_PI )))*fs;
      
      bqelev_l.set_coefficients( (par.omega_up-fs)*inv_a0_u, 0.0, (par.omega_up+alpha_u)*inv_a0_u, (par.omega_up-alpha_u)*inv_a0_u, 0.0);
      bqelev_r.set_coefficients( (par.omega_up-fs)*inv_a0_u, 0.0, (par.omega_up+alpha_u)*inv_a0_u, (par.omega_up-alpha_u)*inv_a0_u, 0.0);
    }
    else
    {
...
void hrtf_t::add_pointsource(const TASCAR::pos_t& prel, double width, const TASCAR::wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t* sd)
{
  data_t* d((data_t*)sd);
  ...
  // time delay in meter (panning parameters: target_tau is reached at end of the block)
...
void hrtf_t::add_diffuse_sound_field(const TASCAR::amb1wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t*)
{
...
TASCAR::receivermod_base_t::data_t* hrtf_t::create_data(double srate,uint32_t fragsize)
{
