...
public:
  ...
  foa_sample_t( float w_, float x_, float y_, float z_ ) : w(w_),x(x_),y(y_),z(z_) {};
  float w;
  float x;
  float y;
  float z;
...
public:
  ...
  inline foa_sample_t& elem00x(uint32_t p2) { 
    return data[p2]; 
  };
  inline const foa_sample_t& elem00x(uint32_t p2) const { 
    return data[p2]; 
  };
  inline void clear() {
    memset(data,0,sizeof(data[0])*s1*s2);
  };
protected:
  uint32_t s1;
  uint32_t s2;
...
public:
  cmat2_t( uint32_t d1 );
  ~cmat2_t();
  inline foa_sample_t& elem(uint32_t p1) { 
    return data[p1]; 
  };
  inline const foa_sample_t& elem(uint32_t p1) const { 
    return data[p1]; 
  };
  inline void clear() {
    memset(data,0,sizeof(data[0])*s1);
  };
protected:
  uint32_t s1;
  foa_sample_t* data;
...
cmat2_t::cmat2_t( uint32_t d1 )
  : s1(d1),
    data(new foa_sample_t[s1])
{
  clear();
}
...
fdn_t::fdn_t(uint32_t fdnorder, uint32_t maxdelay, bool logdelays )
  : logdelays_(logdelays),
    ...
    feedbackmat(fdnorder_ * fdnorder_),
    reflection(fdnorder),
    prefilt(2),
    rotation(fdnorder),
    dlout(fdnorder_),
...
void fdn_t::process(bool b_prefilt)
{
  ...
  for(uint32_t tap=0;tap<fdnorder_;++tap){
    ...
    for(uint32_t otap=0;otap<fdnorder_;++otap)
      delayline.elem(tap,pos[tap]) += dlout.elem(otap)*feedbackmat[fdnorder_*tap+otap];
    // iterate delayline:
    if( !pos[tap] )
      pos[tap] = delay[tap];
    if( pos[tap] )
...
void fdn_t::setpar(float az, float daz, float t, float dt, float g, float damping )
{
  ...
  for(uint32_t tap=0;tap<fdnorder_;++tap){
    ...
    rotation[tap] *= q;
    q.set_rotation( 0.125*daz*(tap % 3)-0.25*daz, TASCAR::pos_t( 1,0,0 ) );
    rotation[tap] *= q;
  }
  // set feedback matrix:
  if( fdnorder_ > 1 ){
    ...
    wgain(MIN3DB)
{
  ...
  distcorr = 1.0/(0.5*pow(volumetric.x*volumetric.y*volumetric.z,0.33333));
  t = dmin;
  dt = (dmax-dmin);
  if( t60 <= 0.0 )
...
void simplefdn_t::set_par( double dw_, double decay_, double damping_ )
{
  dw = dw_;
  t60 = decay_;
  damping = damping_;
  if( pthread_mutex_lock( &mtx ) == 0 ){
    if( fdn ){
      ...
      fdn->setpar(wscale*w,wscale*dw,f_sample*t,f_sample*dt,exp(-4.2*t/t60),std::max(0.0,std::min(0.999,damping)));
    }
