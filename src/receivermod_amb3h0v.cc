...
void amb3h0v_t::add_diffusesource(const TASCAR::amb1wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t* sd)
{
  for( unsigned int i=0;i<chunk.size();i++){
...
uint32_t amb3h0v_t::get_num_channels()
{
...
std::string amb3h0v_t::get_channel_postfix(uint32_t channel) const
{
  char ctmp[32];
