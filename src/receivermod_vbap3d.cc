...
public:
  ...
  void add_pointsource(const TASCAR::pos_t& prel, const TASCAR::wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t*);//declaration add_pointsource
...
void rec_vbap3d_t::add_pointsource(const TASCAR::pos_t& prel, const TASCAR::wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t* sd)
...
void rec_vbap3d_t::add_diffusesource(const TASCAR::pos_t& prel, const TASCAR::amb1wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t* sd)
{
  data_t* d((data_t*)sd);
  ...
  for(unsigned int k=0;k<output.size();k++)
    d->dx[k] = (dot_prod(px,spkpos[k].unitvector) - d->x[k])*d->dt;
  for(unsigned int k=0;k<output.size();k++)
    d->dy[k] = (dot_prod(py,spkpos[k].unitvector) - d->y[k])*d->dt;
  for(unsigned int k=0;k<output.size();k++)
    d->dz[k] = (dot_prod(pz,spkpos[k].unitvector) - d->z[k])*d->dt;
  for( unsigned int i=0;i<chunk.size();i++){
    for( unsigned int k=0;k<output.size();k++){
...
uint32_t rec_vbap3d_t::get_num_channels()
{
  return spkpos.size();
}

std::string rec_vbap3d_t::get_channel_postfix(uint32_t channel) const
{
  char ctmp[1024];
