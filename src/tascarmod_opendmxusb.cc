...
class mod_opendmxusb_t : public artnetdmx_vars_t, public DMX::OpenDMX_USB_t, public TASCAR::service_t {
public:
  mod_opendmxusb_t( const TASCAR::module_cfg_t& cfg );
  virtual ~mod_opendmxusb_t();
...
void mod_opendmxusb_t::setval(uint32_t k,double val)
{
  if( k<objval.size() )
    objval[k] = val;
}

int mod_opendmxusb_t::osc_setval(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg, void *user_data)
{
  mod_opendmxusb_t* h((mod_opendmxusb_t*)user_data);
...
void mod_opendmxusb_t::setw(uint32_t k,double val)
{
  if( k<objw.size() )
    objw[k] = val;
}

int mod_opendmxusb_t::osc_setw(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg, void *user_data)
{
  mod_opendmxusb_t* h((mod_opendmxusb_t*)user_data);
  for( int32_t k=0;k<argc;++k)
    if( types[k] == 'f' )
...
mod_opendmxusb_t::mod_opendmxusb_t( const TASCAR::module_cfg_t& cfg )
  : artnetdmx_vars_t( cfg ),
    DMX::OpenDMX_USB_t(device.c_str())
{
  if( fixtures.size() == 0 )
  ...
  session->add_method("/"+id+"/dmx",std::string(objval.size(),'f').c_str(),&mod_opendmxusb_t::osc_setval,this);
  session->add_method("/"+id+"/w",std::string(objw.size(),'f').c_str(),&mod_opendmxusb_t::osc_setw,this);
  //
  start_service();
}

mod_opendmxusb_t::~mod_opendmxusb_t()
{
  stop_service();
}

void mod_opendmxusb_t::service()
{
...
void mod_opendmxusb_t::update(uint32_t frame,bool running)
{
...
REGISTER_MODULE(mod_opendmxusb_t);
